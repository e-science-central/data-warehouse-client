...
class DataWarehouse:
    ...
    def coreSQLforWhereClauses(self, study:int,participant:int,measurementType:int,measurementGroup:int,groupInstance:int,trial:int,startTime,endTime):
    ...
    def getMeasurements(self, study=-1,participant=-1,measurementType=-1,measurementGroup=-1,groupInstance=-1,trial=-1,startTime=-1,endTime=-1):
       q                  = self.coreSQLforMeasurements()
       (w,conditionCount) = self.coreSQLforWhereClauses(study,participant,measurementType,measurementGroup,groupInstance,trial,startTime,endTime)
       q                  += w
    ...
    def aggregateMeasurements(self, measurementType,aggregation,study=-1,participant=-1,measurementGroup=-1,groupInstance=-1,trial=-1,startTime=-1,endTime=-1):
    ...
    def getMeasurementsWithValueTest(self, measurementType,valueTestCondition,study=-1,participant=-1,measurementGroup=-1,groupInstance=-1,trial=-1,startTime=-1,endTime=-1):
    ...
    def getMeasurementGroupInstancesWithValueTests(self, measurementGroup,valueTestConditions,study=-1,participant=-1,trial=-1,startTime=-1,endTime=-1):
       ...
       (w,conditionCount) = self.coreSQLforWhereClauses(study,participant,-1,measurementGroup,-1,trial,startTime,endTime)
       q += w
       if (conditionCount==0):
          q+= " WHERE ("
       else:
       ...
       for (measurementType,condition) in valueTestConditions:
          mtInfo=self.getMeasurementTypeInfo(measurementType)
          valType = mtInfo[0][2]
          cond = "((measurement.measurementtype = " + str(measurementType) + ") AND " + self.makeValueTest(valType,condition) +")"
          q += cond
